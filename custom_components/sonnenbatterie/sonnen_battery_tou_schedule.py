...
class SonnenBatteryTOUSchedule(CoordinatorEntity, TextEntity):
    def __init__(self, hass, sonnenbatterie:sonnenbatterie, allSensorsPrefix, model_name, async_add_entities, mainCoordinator):
        ...
        self.coordinator = DataUpdateCoordinator(hass, LOGGER, name="Sonnen battery special sensors time of use", update_interval=timedelta(seconds=UPDATE_FREQUENCY_TOU_SCHEDULE), update_method=self.async_handle_coordinator_update)
        ...
        self.LOGGER.warn("SonnenBatteryTOUSchedule added service "+SERVICE_SET_TOU_SCHEDULE)

        self.LOGGER.warn("SonnenBatteryTOUSchedule initialised")
